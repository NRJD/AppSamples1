 ...
 */
public class RegistryDataJaxbUtilsTest {
    // Encoding used.
    private static final Charset UTF8 = StandardCharsets.UTF_8;
    // Registry test data.
    private static final RegistryData REGISTRY_DATA = createReistryData();
    private static final String XML_HEADER = "<?xml version=\"1.0\" encoding=\"" + UTF8.name() + "\" standalone=\"yes\"?>";
     ...
     * Default constructor.
     */
    public RegistryDataJaxbUtilsTest() {
    }
    ...
    public void testGenerateRegistryData_WithValidRegistryData() throws Exception {
        byte[] registryByteData = RegistryDataJaxbUtils.generateRegistryData(REGISTRY_DATA);
        Assert.assertNotNull("The generated registry byte data is expected to be not null", registryByteData);
        String actualRegistryXml = new String(registryByteData, UTF8);
    ...
    public void testGenerateRegistryData_WithNullRegistryData() throws Exception {
        byte[] registryByteData = RegistryDataJaxbUtils.generateRegistryData(null);
        Assert.assertNull("The generated registry byte data is expected to be null", registryByteData);
    }
    ...
    public void testParseRegistryData_WithValidRegistryData() throws Exception {
        RegistryData registryData = RegistryDataJaxbUtils.parseRegistryData(REGISTRY_XML.getBytes(UTF8));
        Assert.assertNotNull("The generated registry data is expected to be not null", registryData);
        byte[] registryByteData = RegistryDataJaxbUtils.generateRegistryData(registryData);
        Assert.assertNotNull("The generated registry byte data is expected to be not null", registryByteData);
        String actualRegistryXml = new String(registryByteData, UTF8);
        System.out.println("expectedRegistryXml: " + REGISTRY_XML);
        System.out.println("  actualRegistryXml: " + actualRegistryXml);
    ...
    public void testParseRegistryData_WithNullRegistryData() throws Exception {
        RegistryData registryData = RegistryDataJaxbUtils.parseRegistryData(null);
